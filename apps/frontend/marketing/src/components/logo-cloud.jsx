  ...
  return (
    <>
      <div className="py-2 relative overflow-hidden">
          ...
          {brandList.map((brand, index) => (
            <div key={index}>
              ...
              <img
                src={brand.lightimg}
                alt={brand.name}
                className="hidden dark:block w-36 h-8 mr-12 lg:mr-20 grayscale hover:grayscale-0"
              />
            </div>
          ))}
        </Marquee>
        ...
        {/* Left blur */}
        <div className="pointer-events-none absolute inset-y-0 left-0 w-1/4 bg-linear-to-r from-zinc-950 to-transparent" />

        {/* Right blur */}
        <div className="pointer-events-none absolute inset-y-0 right-0 w-1/4 bg-linear-to-l from-zinc-950 to-transparent" />
      </div>
    </>
  );
};
