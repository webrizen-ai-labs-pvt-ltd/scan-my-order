...
import { fetchStoreByIdApi, updateMenuItemApi, deleteMenuItemApi } from "../../services/admin-api.js"
...
export default function MenuItemEditPage() {
  ...
  useEffect(() => {
    async function loadItem() {
      if (!token || !storeId || !itemId) return
      setLoading(true)
      try {
        const res = await fetchStoreByIdApi(token, storeId)
        const storeData = res?.data
        const foundItem = storeData?.menuItems?.find((i) => i.id === itemId)
        if (foundItem) {
          setName(foundItem.name || "")
          setDescription(foundItem.description || "")
          setPrice(String(foundItem.price || 0))
          setCategory(foundItem.category || "General")
          setImage(foundItem.image || "")
          setIsAvailable(foundItem.isAvailable !== false)
          setDietaryType(foundItem.dietaryType || "VEG")
          setSpicinessLevel(String(foundItem.spicinessLevel ?? 0))
          setPrepTime(foundItem.prepTime ? String(foundItem.prepTime) : "")
          setCalories(foundItem.calories ? String(foundItem.calories) : "")
          setAllergens(foundItem.allergens || "")
        }
      } catch (err) {
        setMsg({ text: err instanceof Error ? err.message : "Failed to load menu item.", error: true })
      } finally {
        setLoading(false)
      }
    }
    loadItem()
  }, [token, storeId, itemId])
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await updateMenuItemApi(token, storeId, itemId, {
        name,
        description,
      ...
      setMsg({ text: "Menu item updated successfully!", error: false })
      setTimeout(() => navigate(`/dashboard/stores/${storeId}/manage`), 1000)
    } catch (err) {
      setMsg({ text: err instanceof Error ? err.message : "Failed to update menu item.", error: true })
    } finally {
  ...
  const handleDelete = async () => {
    ...
    setIsSubmitting(true)
    try {
      await deleteMenuItemApi(token, storeId, itemId)
      navigate(`/dashboard/stores/${storeId}/manage`)
    } catch (err) {
    ...
    <div className="space-y-8 w-full">
      ...
      <div className="flex items-center justify-between border-b border-zinc-800 pb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-bold text-white tracking-tight">Edit Menu Item</h1>
            <p className="text-xs text-zinc-500">Update your dish details and specifications</p>
          </div>
        </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-16 text-zinc-500 gap-2">
          ...
          <p className="text-xs">Fetching food item parameters...</p>
        </div>
      ) : (
