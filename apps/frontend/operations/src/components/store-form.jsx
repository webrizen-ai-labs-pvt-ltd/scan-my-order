...
import { Loading03Icon, Store01Icon, ArrowLeft02Icon, UserIcon } from 'hugeicons-react';
...
export const StoreForm = ({ initialData, isEdit }) => {
  ...
  const canManageAdmins = currentUser?.role === 'SUPER_ADMIN' || currentUser?.role === 'TENANT_ADMIN';

  useEffect(() => {
    // Fetch users of the current tenant to allow assigning an existing user
    if (currentUser?.tenantId) {
      api.get(`/users?tenantId=${currentUser.tenantId}&limit=1000`)
        .then(res => {
          if (res.data.success) {
    ...
    if (isEdit) {
      ...
      api.get(`/users?role=STORE_MANAGER&storeId=${initialData.id}`)
        .then(res => {
          if (res.data.success) {
            setExistingAdmins(res.data.data.users || res.data.data); 
          }
        })
        .catch(console.error)
        .finally(() => setFetchingAdmins(false));
    }
  }, [isEdit, currentUser?.tenantId, initialData?.id]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="icon" onClick={() => navigate('/dashboard/stores')} className="shrink-0 text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100">
        ...
        <CardContent className="p-6">
          <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <div className="space-y-4 md:col-span-2">
                  <div className="flex items-center justify-between">
                    ...
                    <Button 
                      ...
                      variant="outline" 
                      size="sm" 
                      onClick={() => handleChange('taxRules', [...formData.taxRules, { name: '', rate: 0 }])}
                    >
                      + Add Tax Rule
                    </Button>
                  </div>
                  {formData.taxRules.length === 0 ? (
                    <p className="text-sm text-zinc-500 italic">No taxes configured. (0% tax applied)</p>
                  ) : (
                    <div className="space-y-2">
                      {formData.taxRules.map((rule, idx) => (
                        <div key={idx} className="flex items-center gap-2">
                          <Input 
                            placeholder="e.g. CGST" 
                            value={rule.name} 
                            onChange={(e) => {
                              const newRules = [...formData.taxRules];
                              newRules[idx].name = e.target.value;
                              handleChange('taxRules', newRules);
                            }}
                          />
                          <Input 
                            type="number" 
                            ...
                            onChange={(e) => {
                              const newRules = [...formData.taxRules];
                              newRules[idx].rate = parseFloat(e.target.value) || 0;
                              handleChange('taxRules', newRules);
                            }}
                          />
                          <Button 
                            ...
                            variant="ghost" 
                            onClick={() => {
                              const newRules = formData.taxRules.filter((_, i) => i !== idx);
                              handleChange('taxRules', newRules);
                            }}
            ...
            <div className="flex justify-end pt-4">
              <Button type="button" variant="ghost" onClick={() => navigate('/dashboard/stores')} className="mr-3">
                Cancel
              </Button>
              <Button 
                type="submit" 
